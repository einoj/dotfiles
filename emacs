...
(defun org-color-cells()
    (interactive)
    (let* ((tree (org-element-parse-buffer))
           (tables (org-element-map tree 'table 'identity)))
      (org-element-map (car tables) 'table-cell
         (lambda (cell_string)
           (when (string= (car (org-element-contents cell_string)) "Base")
               (let ((overlay (make-overlay (org-element-property :contents-begin cell_string)
                                            (org-element-property :contents-end cell_string))))
                 (overlay-put overlay 'face '(background-color . "orange"))))
           (when (string= (car (org-element-contents cell_string)) "Tapering")
               (let ((overlay (make-overlay (org-element-property :contents-begin cell_string)
                                            (org-element-property :contents-end cell_string))))
                 (overlay-put overlay 'face '(background-color . "green"))))
           (when (string= (car (org-element-contents cell_string)) "Specific")
               (let ((overlay (make-overlay (org-element-property :contents-begin cell_string)
                                            (org-element-property :contents-end cell_string))))
                 (overlay-put overlay 'face '(background-color . "yellow"))))
           (when (string-prefix-p "K:" (car (org-element-contents cell_string)))
               (let ((overlay (make-overlay (org-element-property :contents-begin cell_string)
                                            (org-element-property :contents-end cell_string))))
...
(add-hook 'org-ctrl-c-ctrl-c-hook 'org-color-cells)
